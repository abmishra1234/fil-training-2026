...
from minibank.exceptions import InsufficientFundsError
...
class InterestStrategy:
    ...
    def apply(self, account: Account) -> List[Transaction]:
        ...
        """
        if account.is_closed or not isinstance(account, InterestBearingAccount):
            return []
        transaction = account.accrue_monthly_interest()
...
class MaintenanceFeeStrategy:
    ...
    def apply(self, account: Account) -> List[Transaction]:
        ...
        if not fee.is_positive:
            return []
        try:
            return [account.withdraw(fee, "monthly maintenance fee")]
        except InsufficientFundsError:
            account.audit("maintenance fee rejected: insufficient funds")
            return []
    ...
    def __init__(self, *strategies: object) -> None:
        ...
        for strategy in strategies:
            if not callable(getattr(strategy, "apply", None)):
                raise TypeError("strategy must provide a callable apply method")
        self._strategies = tuple(strategies)

    @property
    def name(self) -> str:
        """The child names joined with ``"+"``."""
        return "+".join(str(getattr(strategy, "name")) for strategy in self._strategies)

    def apply(self, account: Account) -> List[Transaction]:
        transactions: List[Transaction] = []
        for strategy in self._strategies:
            transactions.extend(strategy.apply(account))
        return transactions
